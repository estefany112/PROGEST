    ...
    @if($role === 'admin')
        <div class="grid md:grid-cols-3 gap-6">
            ...
            <div class="bg-gray-800 p-6 rounded-lg border border-gray-700 shadow hover:shadow-xl transition">
                ...
                <a href="{{ route('admin.usuarios.pendientes') }}" class="text-blue-400 hover:text-blue-300 font-medium">Ver Pendientes de aprobar →</a>
                <br>
                <a href="{{ route('usuarios.index') }}" class="text-blue-400 hover:text-blue-300 text-sm font-medium">Ver todos →</a>
            </div>
        ...
        {{-- Lista de cotizaciones EN REVISIÓN (opcional) --}}
        @if(!empty($cotizaciones) && count($cotizaciones))
        <div class="mt-10">
                        ...
                        @foreach($cotizaciones as $cotizacion)
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2 text-white">
                                    {{ $cotizacion->fecha_emision ? $cotizacion->fecha_emision->format('d/m/Y') : '' }}
                                </td>
                                ...
                                <td class="px-4 py-2 flex flex-wrap gap-2">
                                    ...
                                    <form action="{{ route('cotizaciones.aprobar', $cotizacion) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="text-green-400 hover:text-green-200 font-medium"
                                    ...
                                    <form action="{{ route('cotizaciones.rechazar', $cotizacion) }}" method="POST" class="inline">
                                        @csrf
                                        <input type="text" name="comentario_rechazo" placeholder="Motivo (opcional)"
                                               class="border rounded px-2 py-1 text-xs mr-1 bg-gray-900 border-gray-700 text-gray-100" style="width:140px;">
                                        <button type="submit" class="text-red-400 hover:text-red-200 font-medium"
                                            onclick="return confirm('¿Rechazar esta cotización?')">Rechazar</button>
                                    </form>
                                </td>
                            </tr>
                            @endif
