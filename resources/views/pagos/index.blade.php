...
<div class="max-w-7xl mx-auto py-10 px-6">
    ...
    {{-- Filtro por estado --}}
    <form method="GET" action="{{ route('pagos.index') }}" class="mb-6 flex items-center space-x-3">
                ...
                <table class="min-w-full bg-gray-900 rounded-lg">
                    <thead class="bg-gray-800">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-sm text-gray-300 uppercase">Contraseña de Pago</th>
                            ...
                            <tr class="hover:bg-gray-800 transition">
                                ...
                                <td class="px-6 py-4 text-white">
                                    ...
                                    <div class="text-sm text-gray-400">
                                        OC: {{ $pago->factura->ordenCompra->numero_oc ?? 'N/A' }}
                                    </div>
                                </td>
                                ...
                                {{-- Contraseña de Pago --}}
                                <td class="px-6 py-4 text-white">
                                    @if($pago->contrasenaPago)
                                        <span class="text-blue-400 font-semibold">
                                            {{ $pago->contrasenaPago->codigo }}
                                        </span>
                                    @else
                                        <span class="text-gray-500">Sin contraseña</span>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4 text-sm font-medium">
                                    <div class="flex flex-wrap justify-center gap-2 items-center">
                                        ...
                                        <form action="{{ route('pagos.cambiarEstado', $pago->id) }}" method="POST">
                                            ...
                                            @role('asistente')
                                                <select disabled class="px-2 py-1 border border-gray-700 bg-gray-900 text-gray-100 rounded-md text-xs">
                                                    <option value="pendiente" {{ $pago->status == 'pendiente' ? 'selected' : '' }}>Pendiente</option>
                                                    <option value="pagada" {{ $pago->status == 'pagada' ? 'selected' : '' }}>Pagada</option>
                                                </select>
                                            @endrole
                                       ...
                                       @role('admin')
                                        <form id="form-delete-{{ $pago->id }}" 
                                            ...
                                            <button type="button"
                                                            onclick="confirmarEliminacion({{ $pago->id }}, '{{ $pago->codigo ?? 'P-' . $pago->id }}')"
...
<script>
  function confirmarEliminacion(id, numero_factura) {
    Swal.fire({
      title: '¿Eliminar pago?',
      html: `<p>El pago <strong>${numero_factura}</strong> será eliminado permanentemente.</p>`,
      icon: 'warning',
